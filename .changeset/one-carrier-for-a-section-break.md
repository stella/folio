...
Collaboration snapshots carry an attr-schema version, bumped to 4. A snapshot written under version 3 that states `sectionBreakType` and no record has the record minted for it on load; without the step ProseMirror would drop the attr its schema no longer declares and the save would write one `w:sectPr` fewer.

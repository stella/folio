/**
 * Set-diff + formatted report helpers shared by the parity gate scripts.
 *
 * `diffSets(left, right, allowed?)` returns the symmetric difference, optionally
 * filtered through an opt-out set. `formatDiff(opts)` writes a report block to
 * the console and returns whether the gate failed.
 *
...
export function formatDiff({
  ...
  const sym = strict ? "x" : "i";
  const stream = strict ? console.error : console.log;
  stream(
    `${sym} ${label}: drift (${leftOnly.length} ${leftLabel}, ${rightOnly.length} ${rightLabel})`,
  );
  if (!strict) {

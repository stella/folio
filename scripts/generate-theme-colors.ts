...
import { buildIndex, loadSchemaGraph, type SchemaGraph } from "./lib/ooxml-schema-graph";

const REPO_ROOT = path.resolve(import.meta.dir, "..");
const OUTPUT_PATH = path.join(REPO_ROOT, "packages/docx-core/src/model/themeColor.gen.ts");
const SCRIPT = "generate:theme-colors";
...
const main = async (): Promise<void> => {
  ...
  const rendered = renderModule({
    summary: "The colour enumerations a theme reference passes through.",
    script: SCRIPT,
    lists: [
      renderList({
        name: "THEME_COLORS",
 ...
 */`,
        members: schemeColorSlots,
      }),
      renderList({
        name: "SCHEME_COLOR_VALUES",
 ...
 */`,
        members: schemeColorValues,
      }),
      renderList({
        name: "CLR_SCHEME_MAPPING_KEYS",
 ...
 */`,
  ...
  await emitGeneratedModule({
    ...
    rendered,
    summary:
      `${String(themeColors.length)} theme colours, ` +

 ...
 * `@stll/folio-nuxt`
 *
 * Nuxt 3 & 4 module that wraps the `@stll/folio-vue` adapter so Nuxt apps get a
 * zero-config, SSR-safe `<DocxEditor>` component plus auto-imported composables.
 *
...
// static list because the runtime bundle can't be imported in the module's Node
// context (it transitively imports a `.css` file). When the adapter adds a
// composable to that barrel, add its name here too — keep this in sync with
...
/**
 * Configuration for the folio DOCX editor Nuxt module, read from the
...
export type ModuleOptions = {
  /**
   * Component name prefix. With `prefix: 'Folio'` the component is
   * auto-imported as `<FolioDocxEditor>` instead of `<DocxEditor>`. Defaults to
   * `''`.
   */
  prefix?: string;
  /**
   * Whether the module pushes the editor stylesheet into `nuxt.options.css`.
   * Set to `false` to import `@stll/folio-vue/editor.css` yourself. Defaults to
   * `true`.
   */
...
const module: NuxtModule<ModuleOptions> = defineNuxtModule<ModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    optimizeDeps.include = [...(optimizeDeps.include ?? []), "@stll/folio-core", "@stll/folio-vue"];

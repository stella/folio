...
describe("folio executable", () => {
  test("read -> suggest -> changes -> accept --all -> compare", async () => {
    ...
    const accepted = folio(["accept", file, "--all", "--in-place"]);
    ...
    const written = folio(["compare", original, file, "-o", redline]);
  ...
  test("refuses a write with no author configured", async () => {
    const file = await writeDocx(dir, "unowned.docx", CONTRACT_PARAGRAPHS);

    const run = folio(["comment", file, "--block-id", "10000002", "--text", "?", "--in-place"], {

 ...
 *
 * NOTE (i18n): the `nameKey` / `labelKey` values below reference catalog keys
 * (`styles.*`, `lineSpacing.*`) that are not yet present in folio's `folio`
 * message catalog. `TranslationKey` is typed as `string` here (our
 * `useTranslation().t` accepts a plain key path); the localized labels resolve
 * once those keys are added to `@stll/folio-core/i18n/messages`.
 */
...
export const paragraphStyles: ParagraphStylePreset[] = [
  { id: "Normal", label: "Normal", nameKey: "styles.normalText" },
  { id: "Title", label: "Title", nameKey: "styles.title" },
  { id: "Subtitle", label: "Subtitle", nameKey: "styles.subtitle" },
  { id: "Heading1", label: "Heading 1", nameKey: "styles.heading1" },
  { id: "Heading2", label: "Heading 2", nameKey: "styles.heading2" },

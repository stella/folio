...
const tabStopPositions = computed(() => {
  ...
  return props.tabStops.map((ts) => {
    // Our `TabStop` model carries the twip offset in `position`.
...
// Drag handlers: dragStart value is captured per-marker so the tooltip
// reflects the in-progress value while the user drags. (The move handler
// derives position from the ruler rect, so no drag-start X is needed.)

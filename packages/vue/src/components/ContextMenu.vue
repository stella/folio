<!--
  ...
  PORT-BLOCKED: `@stll/folio-core` has no `types/agentApi` — the AI-action
  schema (`AIAction`, `SelectionContext`, `DEFAULT_AI_ACTIONS`) is an
  upstream-only subsystem absent from our core. A minimal local `AIAction`
  union + `DEFAULT_AI_ACTIONS` list mirror the ten upstream actions so the
  menu compiles and stays functional; swap these for the core exports once
  the agent layer is ported.
-->
<template>
  <div
    v-if="isOpen"
...
// PORT-BLOCKED stub for the absent `@stll/folio-core/types/agentApi` schema.
type AIAction =
  | "askAI"
  | "rewrite"
  | "expand"
...
function onDocClick(e: MouseEvent) {
  ...
  if (target instanceof HTMLElement && !target.closest(".docx-context-menu")) emit("close");
}

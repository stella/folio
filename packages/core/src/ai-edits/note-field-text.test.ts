/**
 * Field results in the notes projection: a footnote's PAGE or DATE field must
 * read as the result stored in the package, never as the placeholder the editor
 * paints for an unresolved field — one branch of which is the current date, so
 * the same document read twice could otherwise disagree.
 */
...
/**
 * Note text only comes from an editor document once the story has been loaded;
 * an untouched note is read straight off the parsed package. Load every note so
 * the assertions cover the editor-document path.
 */
const openWithLoadedNotes = async (buffer: ArrayBuffer): Promise<FolioDocxReviewer> => {
  const reviewer = await FolioDocxReviewer.fromBuffer(buffer);
  for (const { handle } of reviewer.listStories()) {
    if (handle.type === "footnote") {
...
describe("field results in note text", () => {
  test("reads a note's fields as their stored results", async () => {
    const reviewer = await openWithLoadedNotes(await createNoteFieldDocx());
    const notes = reviewer.getNotesAsText();

    expect(notes).toContain(`[footnote #${NOTE_WITH_RESULTS_ID}] page 7 dated 1 January 2020`);
    expect(notes).not.toContain("{page}");
    expect(notes).not.toContain(new Date().toLocaleDateString());
  });

  test("reads the same document the same way twice", async () => {
    const buffer = await createNoteFieldDocx();
    const reviewer = await openWithLoadedNotes(buffer);
    const reopened = await openWithLoadedNotes(buffer);
  ...
  test("adds nothing for a note field with no stored result", async () => {
    ...
    expect(notes).toContain(`[footnote #${NOTE_WITHOUT_RESULT_ID}] page dated end`);
    expect(notes).not.toContain("{PAGE}");
    expect(notes).not.toContain("{page}");
  });
  ...
  test("leaves a note without a field byte-for-byte unchanged", async () => {
    const buffer = await createNoteFieldDocx();
    const loaded = await openWithLoadedNotes(buffer);

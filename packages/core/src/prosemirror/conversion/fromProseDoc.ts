...
const keepOneRecordCarriedByIdentity = (blocks: readonly BlockContent[]): void => {
  ...
  /** `true` when this record is the first in document order to present it. */
  const claim = (carried: object): boolean => {
    if (seen.has(carried)) {
      return false;
    }
    seen.add(carried);
  ...
  const keepRemainder = (record: { preservedAttributes?: PreservedAttribute[] }): void => {
    const remainder = record.preservedAttributes;
    if (remainder !== undefined && !claim(remainder)) {
      delete record.preservedAttributes;
    }
  };
  const keepSink = (record: { preserved?: PreservedMarkup }): void => {
    const sink = record.preserved;
    if (sink !== undefined && !claim(sink)) {

...
describe("document operation contract", () => {
  test("reports the supported version, operations, modes, and stories", () => {
    expect(getFolioDocumentOperationCapabilities()).toEqual({
      ...
      modesByOperationType: {
        ...
        splitBlock: ["direct", "tracked-changes"],
        mergeBlockWithNext: ["direct", "tracked-changes"],
        setBlockParagraphProperties: ["direct", "tracked-changes"],
        insertTable: ["direct", "tracked-changes"],
        deleteTable: ["direct", "tracked-changes"],
  ...
  test("reports mode support for each operation type", () => {
    ...
    expect(isFolioDocumentOperationModeSupported("deleteTableColumn", "tracked-changes")).toBe(
      true,
    );

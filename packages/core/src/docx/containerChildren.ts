 ...
 *
 * `<w:lock w:val="sdtUnlocked"/>` states a value a reader's enumeration does
 * not admit, and `<w:id/>` states no number. Neither can be decided by name: a
 * map keyed by name cannot list the values a reader will refuse, only the
 * names it has never heard of. A handler that reads a *projection* off a child
 * it does not model whole answers `CAPTURE` too — it took a value, not the
 * element.
 */
 ...
 * — "I looked and kept no bytes of this, so keep them". A property set needs
 * it: `<w:lock w:val="sdtUnlocked"/>` states a value the reader's enumeration
 * does not admit, and a handler that turns it into no typed value leaves the
 * element with nowhere to go. Deciding it by the outcome rather than by the
 * name is what makes the decision total: the map cannot list the values a
 * reader will refuse.
 */
 ...
 *
 * A property set is that container: `CT_SdtPr` is a sequence of optional
 * singletons closed by a choice, so a child's position is a property of its
 * name and the generated list is the order. That is what the sink records for
 * them, rather than a count of modelled siblings: the count is a mirror of
 * whichever properties folio models today, and it moves under the capture the
 * moment one more of them is modelled.
 *
 * An undeclared child — `w14:checkbox`, `w15:appearance`, an `mc:` construct, a
 * name a later revision of the format adds — has no place in the sequence, so
 * it takes the place of the last declared child before it and comes back
 * beside the same neighbour.
 */
 ...
 * drifted from the schema before, and Word repairs a property set whose
 * children are out of order. The captured half carries the ordinal the parser
 * read it at (see {@link sequencePositions}), so the two merge by the same key
 * and a capture lands where the source put it.
 *

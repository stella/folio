...
test.each([" PAGE ", "  PAGE   \\* MERGEFORMAT  ", ' MERGEFIELD "A B" \\* MERGEFORMAT '])("preserves authored field instruction spacing across review saves: %s", async (instruction) => {
  const document = createEmptyDocument();
  document.package.document.content = [{type: "paragraph", content: [{type: "complexField", instruction, fieldCode: [], fieldResult: [{type: "run", content: [{type: "text", text: "1"}]}]}]}];

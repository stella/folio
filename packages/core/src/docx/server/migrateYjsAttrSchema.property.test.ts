...
/** The union a stored version-4 slot pair carries forward to, or absent. */
...
const versionFourAttrs: StoredAttrsOf = ({ numPr, outlineLevel }) => ({
...
const syntheticSnapshot = (blocks: readonly SyntheticParagraph[]): SyntheticSnapshot => {
  const document = proseDocument(blocks, versionFourAttrs);

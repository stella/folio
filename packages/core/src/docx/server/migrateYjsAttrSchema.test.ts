...
/**
 * A version-4 snapshot holding one row, with the `hidden` a version-4 build
 * stored: `false` for a row that authored no `w:hidden`, because the reader
 * that filled it took only an explicit on.
 */
const versionFourRowSnapshot = (): Uint8Array => {
  const ydoc = new Y.Doc();
  const cell = new Y.XmlElement("tableCell");
  cell.insert(0, [new Y.XmlElement("paragraph")]);
  const row = new Y.XmlElement("tableRow");
...
describe("migrateFolioYjsSnapshot carries a version-4 row forward", () => {
  test("drops the `hidden` version 4 could not have stated", () => {
    const migrated = migrateFolioYjsSnapshot(versionFourRowSnapshot());
    if (migrated.isErr()) {
      throw migrated.error;
    }

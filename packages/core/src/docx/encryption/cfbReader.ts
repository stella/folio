/**
 * Minimal read-only MS-CFB reader for password-protected OOXML.
 *
 * Encrypted .docx files only need `/EncryptionInfo` and `/EncryptedPackage`
 * streams from the compound file root.
 */
...
const ensureAvailable = (bytes: Uint8Array, offset: number, length: number, label: string): void => {
  if (offset < 0 || length < 0 || offset + length > bytes.length) {
    throw new Error(`CFB ${label} exceeds available data`);
  }
};

const parseHeader = (bytes: Uint8Array): CfbHeader => {
...
const readSector = (bytes: Uint8Array, header: CfbHeader, sectorId: number): Uint8Array => {
...
const readDifatEntries = (bytes: Uint8Array, header: CfbHeader): number[] => {
  ...
  if (difatEntries.length < header.numberOfFatSectors) {
    throw new Error(
      `CFB DIFAT only exposed ${difatEntries.length} FAT sectors, expected ${header.numberOfFatSectors}`,
    );
  }
...
const readFat = (bytes: Uint8Array, header: CfbHeader): number[] => {
...
const parseStreamSize = (view: DataView, offset: number): number => {
  const streamSize = readUint64LE(view, offset);
  if (streamSize > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`CFB stream is too large: ${streamSize.toString()}`);
  }
  return Number(streamSize);
};

const parseDirectoryEntries = (directoryBytes: Uint8Array): CfbDirectoryEntry[] => {
  const entries: CfbDirectoryEntry[] = [];

  for (
    let offset = 0;
    offset + CFB_DIRECTORY_ENTRY_SIZE <= directoryBytes.length;
...
const forEachSiblingTree = (
  ...
  if (forEachSiblingTree(entries, entry.leftSiblingId, visitor, visited)) {
    return true;
  }
  if (visitor(entry) === true) {
    return true;
  }
  if (forEachSiblingTree(entries, entry.rightSiblingId, visitor, visited)) {
    return true;
  }
  return false;
};
...
const findEntryByPath = (
  ...
  for (const component of components) {
    if (
      currentEntry.objectType !== CFB_OBJECT_TYPE.ROOT &&
      currentEntry.objectType !== CFB_OBJECT_TYPE.STORAGE
    ) {
      return null;
    }
...
const readMiniStream = (
  ...
  return readSectorChain(bytes, header, fat, rootEntry.startSector).subarray(
    0,
    rootEntry.streamSize,
  );
};

const readMiniFat = (bytes: Uint8Array, header: CfbHeader, fat: number[]): number[] => {
...
export type CfbReader = {
  getStream: (path: string) => Uint8Array | null;
};

export const createCfbReader = (data: ArrayBuffer | Uint8Array): CfbReader => {
  const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);
  const header = parseHeader(bytes);
  const fat = readFat(bytes, header);
  const directoryEntries = parseDirectoryEntries(
    readSectorChain(bytes, header, fat, header.firstDirectorySector),
  );

  const rootEntry = directoryEntries[0];
  if (!rootEntry || rootEntry.objectType !== CFB_OBJECT_TYPE.ROOT) {

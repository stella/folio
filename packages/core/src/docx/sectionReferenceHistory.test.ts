...
test("section history resolves an alternate namespace prefix", () => {
  const parsed = parse(
    `<w:sectPrChange w:id="7" w:author="Reviewer"><w:sectPr/><x:previousReferences xmlns:x="urn:stella:folio:section-reference-history:1"><w:footerReference w:type="even" r:id="rId2"/></x:previousReferences></w:sectPrChange>`,
  );
  expect(parsed.propertyChanges?.at(0)?.previousReferences).toEqual({
    footerReferences: [{ type: "even", rId: "rId2" }],
  });
});
...
test("malformed history refuses duplicate selections, invalid kinds, and missing relationships", () => {
  for (const children of [
    ...
    expect(() =>
      parse(
        `<w:sectPrChange w:id="7"><w:sectPr/><x:previousReferences xmlns:x="urn:stella:folio:section-reference-history:1">${children}</x:previousReferences></w:sectPrChange>`,
      ),
    ).toThrow(InvalidSectionReferenceHistoryError);
  }
  expect(() =>
    parse(
      '<w:sectPrChange w:id="7" xmlns:x="urn:stella:folio:section-reference-history:1"><w:sectPr/><x:previousReferences/><x:previousReferences/></w:sectPrChange>',
    ),
  ).toThrow(InvalidSectionReferenceHistoryError);
});

...
/**
 * `CT_Shd`. The pattern colour is read from `w:color` alone: `w:themeColor`,
 * `w:themeTint` and `w:themeShade` describe it in the schema and no reader
 * takes them, so they belong to the remainder. The fill's three theme
 * attributes are spelled `w:themeFill*` and are read.
 */

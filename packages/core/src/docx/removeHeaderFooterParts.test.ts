...
test.each(["retired", "shared"] as const)(
  "resolved references remove only exclusively owned parts (%s)",
  async (ownership) => {
    const zip = new JSZip();
    ...
    zip.file(
      ...
      `<pkg:Relationships xmlns:pkg="${RELS}"><pkg:Relationship Id="rId_header" Type="${RELATIONSHIP_TYPES.header}" Target="header1.xml"/><pkg:Relationship Id="rId_styles" Type="${RELATIONSHIP_TYPES.styles}" Target="styles.xml"/></pkg:Relationships>`,
    );
    zip.file(
      "[Content_Types].xml",
      `<ct:Types xmlns:ct="${TYPES}"><ct:Override PartName="/word/header1.xml" ContentType="header"/><ct:Override PartName="/word/styles.xml" ContentType="styles"/></ct:Types>`,
    );
    if (ownership === "shared") {
      zip.file(
        "customXml/_rels/item1.xml.rels",
        `<Relationships xmlns="${RELS}"><Relationship Id="shared" Type="urn:example:shared" Target="../word/header1.xml"/></Relationships>`,
      );
    }
    ...
    await removeResolvedHeaderFooterParts({
      document,
      zip,
      removedReferences: [{ part: "header", type: "default", relationshipId: "rId_header" }],
      compressionLevel: 6,
    });

...
const findAllByClass = (element: StubElement, className: string): StubElement[] => {
  const matches = element.className.split(" ").includes(className) ? [element] : [];
  return matches.concat(element.children.flatMap((child) => findAllByClass(child, className)));
};
...
const findAllRegions = (regions: readonly DisplayHitRegion[]): DisplayHitRegion[] =>
  regions.flatMap((region) => [region, ...findAllRegions(region.children)]);
...
describe("renderDisplayListToDom", () => {
  test("keeps editing regions inside a clipped table-row continuation", () => {
    withFakeTextMeasure(
      () => {
        ...
        const continuationPageIndex = list.pages.findIndex(
          (page, pageIndex) =>
            pageIndex > 0 && page.primitives.some((primitive) => primitive.kind === "clipGroup"),
        );
        expect(continuationPageIndex).toBeGreaterThan(0);
        const continuation = list.pages[continuationPageIndex];
        expect(continuation).toBeDefined();
        if (continuation === undefined) {
          return;
        }
        const clipPrimitive = continuation.primitives.find(
          (primitive) => primitive.kind === "clipGroup",
        );
        expect(clipPrimitive?.kind).toBe("clipGroup");
        if (clipPrimitive?.kind !== "clipGroup") {
          return;
        }
        const tableRegion = findAllRegions(continuation.regions).find(
          (region) => region.kind === "table",
        );
        expect(tableRegion).toBeDefined();
        if (tableRegion === undefined) {
          return;
        }
        const clippedLineRegion = findAllRegions(clipPrimitive.regions).find(
          (region) => region.kind === "line" && region.model?.blockId === "body-paragraph",
        );
        const clippedRowRegion = clipPrimitive.regions.find(
          (region) => region.kind === "tableRow" && region.model?.rowIndex === 1,
        );
        const clippedParagraphRegion = findAllRegions(clipPrimitive.regions).find(
          (region) => region.kind === "paragraph" && region.model?.blockId === "body-paragraph",
        );
        expect(clippedRowRegion).toBeDefined();
        expect(clippedParagraphRegion).toBeDefined();
        expect(clippedLineRegion?.model?.pmRange).toBeDefined();
        if (
          clippedRowRegion === undefined ||
          clippedParagraphRegion === undefined ||
          clippedLineRegion?.model?.pmRange === undefined
        ) {
          return;
        }

        const rendered = asStub(
          renderDisplayPageToDom(continuation, {
            doc: stubDocument(),
            fonts: list.fonts,
            images: list.images,
        ...
        expect(row?.style.left).toBe(`${clippedRowRegion.rect.xPx - clipPrimitive.rect.xPx}px`);
        expect(row?.style.top).toBe(`${clippedRowRegion.rect.yPx - clipPrimitive.rect.yPx}px`);
        ...
        expect(line?.dataset).toMatchObject({
          blockId: "body-paragraph",
          pmStart: String(clippedLineRegion.model.pmRange.start),
          pmEnd: String(clippedLineRegion.model.pmRange.end),
          story: "body",
        });
        expect(line?.style.left).toBe(
          `${clippedLineRegion.rect.xPx - clippedParagraphRegion.rect.xPx}px`,
        );
        expect(line?.style.top).toBe(
          `${clippedLineRegion.rect.yPx - clippedParagraphRegion.rect.yPx}px`,
        );
